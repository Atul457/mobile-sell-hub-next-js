...
const schema = yup.object().shape({
  ...
  directors: yup
    .array()
    .of(directorSchema)
    .max(3, 'You can add up to 3 directors')
    .nullable(),
...
export default function MultiStepStoreSignup() {
  ...
  const { fields: directorsFields, append: appendDirector, remove: removeDirector } = useFieldArray({
    control,
    name: 'directors'
  })
                ...
                InputProps={{
                  endAdornment: (
                    <InputAdornment position="end">
                      ...
                      >
                        <i className={clsx(!isPasswordShown ? 'tabler-eye-off' : 'tabler-eye')} />
                      </IconButton>
                    </InputAdornment>
                  )
                }}
                  ...
                  endAdornment: (
                    <InputAdornment position="end">
                      <IconButton
                        ...
                        onMouseDown={e => e.preventDefault()}
                      >
                        <i className={clsx(!isCPasswordShown ? 'tabler-eye-off' : 'tabler-eye')} />
                      </IconButton>
                    </InputAdornment>
                  )
                }}
      ...
      {activeStep === 2 && (
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
          ...
          {directorsFields.length === 0 && (
            <Typography sx={{ color: '#000', opacity: 0.8 }}>No directors added yet. Add if applicable.</Typography>
          )}
                  ...
                  <Controller
                    ...
                    render={({ field }) => (
                      <CustomTextField
                        ...
                        helperText={(errors as any)?.directors?.[idx]?.middleName?.message || ''}
                      />
                    )}
                  />
                </Grid>
      ...
      {activeStep === 3 && (
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
          ...
          {((errors as any)?.subscription?.termsAccepted?.message) && (
            <Typography sx={{ color: 'red' }}>{(errors as any)?.subscription?.termsAccepted?.message}</Typography>
          )}
        </Box>
      )}
