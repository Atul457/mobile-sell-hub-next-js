...
type FormData = (typeof commonSchemas.addProductCategory)['__outputType'];
...
const ProductCategoryDrawer = (props: IProductCategoryProps) => {
    const { productCategory, create } = props;
    ...
    const {
        ...
        resolver: yupResolver(commonSchemas.addProductCategory),
    ...
    const onSubmit: SubmitHandler<FormData> = async (data) => {
        try {
            if (!productCategory && !props.create) throw utils.CONST.RESPONSE_MESSAGES.SOMETHING_WENT_WRONG;

            setLoading(true);

            let response = utils.generateRes({ status: true });

            const ps = new ProductCategoryService();
            if (productCategory?._id) {
                response = await ps.update(productCategory?._id as string, data);
                props.onUpdate(response.data?.productCategory);
            } else {
                response = await ps.create(data);
                props.onCreate(response.data?.productCategory);
            }
    ...
    return (
        <Drawer
            anchor='right'
            open={Boolean(props.productCategory || props.create)}
            onClose={onClose}
            ModalProps={{
                disablePortal: true,
            ...
            className={classNames('block', {
                static: !props.productCategory,
                absolute: Boolean(props.productCategory || props.create)
            })}
            PaperProps={{
        ...
        >
            <form noValidate autoComplete='off' className='w-full'>
                ...
                >
                    ...
                    >
                        {props.create ? 'Create' : 'Update'} Product Category
                    </Typography>
                </Box>
