...
const NUMERIC_STATUS = utils.CONST.PRODUCT_CATEGORY.NUMERIC_STATUS;

type IProductCategoriesColumns = {
    onEditClick: (category: IProductCategory) => void;
    permissions: ReturnType<typeof utils.helpers.role.getPermissions>;
};

export const productCategoriesColumns = (props: IProductCategoriesColumns): GridColDef<IProductCategory>[] => {
    return helpers.getDataGridColumnsWithSpaces([
        {
            ...
            renderCell: (params) => (
                ...
                >
                    <Link href={`/portal/product-categories/${params.row.id}`} className='custom-link primary'>
                        <Typography
                            variant='body2'
                            className='ellipsis'
                            sx={{
            ...
            renderCell: (params) => (
                ...
                >
                    <Typography
                        ...
                        title={utils.helpers.getValue(params.row.description)}
                    >
                        {utils.helpers.getValue(params.row.description)}
                    </Typography>
                </Box>
            )
        },
        ...
        ...((props.permissions.update
            ...
            : []) as GridColDef<IProductCategory>[])
    ] as GridColDef<IProductCategory>[]);
};
