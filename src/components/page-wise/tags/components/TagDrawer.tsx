...
type FormData = (typeof commonSchemas.addTag)['__outputType'];
...
const TagDrawer = (props: ITagProps) => {
    const { tag, create } = props;
    ...
    const {
        ...
        resolver: yupResolver(commonSchemas.addTag),
    ...
    const onSubmit: SubmitHandler<FormData> = async (data) => {
        try {
            ...
            const ps = new TagService();
            if (tag?._id) {
                response = await ps.update(tag?._id as string, data);
                props.onUpdate(response.data?.tag);
            } else {
    ...
    return (
        <Drawer
            anchor='right'
            open={Boolean(props.tag || props.create)}
            onClose={onClose}
            ModalProps={{
                disablePortal: true,
            ...
            className={classNames('block', {
                static: !props.tag,
                absolute: Boolean(props.tag || props.create)
            })}
            PaperProps={{
        ...
        >
            <form noValidate autoComplete='off' className='w-full'>
                ...
                >
                    ...
                    >
                        {props.create ? 'Create' : 'Update'} Tag
                    </Typography>
                </Box>
