...
type IActiveSlug = 'intro' | 'story' | 'goal' | 'rewards' | 'length' | 'launch' | 'faq' | 'community' | 'terms';
...
const GameSchema: Schema<IGame> = new Schema(
    {
        ...
        currentSlug: {
            type: String,
            enum: ['intro', 'story', 'goal', 'rewards', 'length', 'launch', 'faq', 'community', 'terms'],
            required: true
        },
        lastCompletedSlug: {
            type: String,
...
const GameModel = (models?.Game as Model<IGame>) || model<IGame>('Game', GameSchema, 'games');
